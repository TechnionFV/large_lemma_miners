module gulwani_cegar1 #(parameter WIDTH = 32) (
    ...
    assign cond = (x >= 0 && x <= 13 && y >= 0 && y <= 17);
   
    assign cond_raw =  (x_init >= 0 && x_init <= 13 && y_init >= 0 && y_init <= 17);
   
    assign assumptions_hold = (x_init >= 0 && x_init <= 13 && y_init >= 0 && y_init <= 17);
    ...
    property prop;
        @(posedge clk) disable iff (rst) ((assumptions_hold && y == 77) |-> x <= 90); 
    endproperty
