module gulwani_cegar1 #(parameter WIDTH = 32) (
    ...
    assign cond = (x >= 0 && x <= 21 && y >= 0 && y <= 21);
   
    assign cond_raw =  (x_init >= 0 && x_init <= 21 && y_init >= 0 && y_init <= 21);
   
    assign assumptions_hold = (x_init >= 0 && x_init <= 21 && y_init >= 0 && y_init <= 21);
    ...
    property prop;
        @(posedge clk) disable iff (rst) ((assumptions_hold && y == 15) |-> x < 41); 
    endproperty
